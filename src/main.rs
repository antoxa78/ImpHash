...
fn add_dir_row(
    ...
    remove_btn.connect_clicked(move |_| {
        ...
        d.retain(|x| *x != t);
        dir_refs2.lock().unwrap().remove(&t);
...
fn build_ui(app: &libadwaita::Application) {
    ...
    provider.load_from_string(
        ".ref-row { background: alpha(@accent_color, 0.08); border-left: 3px solid @accent_color; }\
         ...
         .deleted { background: alpha(@error_color, 0.08); border-left: 3px solid @error_color; }\
         ...
         .result-row { padding: 4px 6px; border-radius: 4px; transition: background 100ms ease; }\
    ...
    select_by_btn.set_label("Select By");
    ...
    let popover_grid = gtk4::Box::new(gtk4::Orientation::Vertical, 4);
    popover_grid.set_margin_start(8);
    popover_grid.set_margin_end(8);
    popover_grid.set_margin_top(8);
    popover_grid.set_margin_bottom(8);
    ...
    let add_size_row = |label: &str, mode_base: i32| {
        let row = gtk4::Box::new(gtk4::Orientation::Horizontal, 12);
        let lbl = gtk4::Label::new(Some(label));
        lbl.set_width_chars(10);
        lbl.set_halign(gtk4::Align::Start);
        let big = gtk4::Button::with_label("Big");
        big.connect_clicked(clone!(#[strong] selection, #[strong] results_data,
            #[strong] status_label, move |_| {
            let data = results_data.lock().unwrap();
            apply_select_by(&data, &selection, mode_base, &status_label);
        }));
        let small = gtk4::Button::with_label("Small");
        small.connect_clicked(clone!(#[strong] selection, #[strong] results_data,
            #[strong] status_label, move |_| {
            let data = results_data.lock().unwrap();
    ...
    popover_grid.append(&add_size_row("Size:", 0));
    popover_grid.append(&add_size_row("Resolution:", 2));
    popover_grid.append(&add_size_row("Path:", 4));
        ...
        select_all_btn.connect_clicked(move |_| {
            if ref_dirs_clone.lock().unwrap().is_empty() {
                let dialog = gtk4::AlertDialog::builder()
                    .message("No reference directory configured")
                    .detail("All files will be selected because no reference directory is set. What would you like to do?")
                    .buttons(["Select Reference Path", "Continue", "Cancel"])
                    .build();
                ...
                dialog.choose(Some(&win2), None::<&gtk4::gio::Cancellable>, move |result| {
                    match result {
                        Ok(0) => {
                            ...
                            if dir_list_snapshot.is_empty() {
                                let info = gtk4::AlertDialog::builder()
                                    .message("No directories added")
                                    .detail("Add directories using the \"Add Directory\" button first, then mark one as Reference.")
                                    .buttons(["OK"])
                                    .build();
                                info.choose(Some(&win2b), None::<&gtk4::gio::Cancellable>, |_| {});
                            } else {
                                ...
                                let sl3 = sl2.clone();
                                let win3 = win2b.clone();
                                // Build a modal window with a dropdown of directories
                                let pick_win = gtk4::Window::new();
                                pick_win.set_title(Some("Select Reference Directory"));
                                pick_win.set_modal(true);
                                pick_win.set_transient_for(Some(&win3));
                                pick_win.set_default_size(500, 200);
                                let vbox = gtk4::Box::new(gtk4::Orientation::Vertical, 12);
                                vbox.set_margin_top(12);
                                vbox.set_margin_bottom(12);
                                vbox.set_margin_start(12);
                                vbox.set_margin_end(12);
                                let lbl = gtk4::Label::new(Some("Choose a directory to mark as reference:"));
                                lbl.set_halign(gtk4::Align::Start);
                                ...
                                let ok_btn = btn_icon_text("Select", "emblem-ok-symbolic");
                                ok_btn.set_css_classes(&["suggested-action"]);
                                let cancel_btn = btn_icon_text("Cancel", "process-stop-symbolic");
                                // OK: mark as ref, update checkbox, select all, close
                                let dd = dropdown.clone();
                                let dir_snap2 = dir_list_snapshot.clone();
                                let pw = pick_win.clone();
                        ...
                        Ok(1) => {
                            ...
                            let confirm = gtk4::AlertDialog::builder()
                                ...
                                .detail("No reference directory is set. All files will be selected. Do you want to continue?")
                            ...
                            let sl4 = sl2.clone();
                            confirm.choose(Some(&win2b), None::<&gtk4::gio::Cancellable>, move |result| {
    ...
    del_cache_btn.connect_clicked(clone!(#[strong] cache, #[strong] window, move |_| {
        let confirm = gtk4::AlertDialog::builder()
            .message("Delete cached hash database?")
            .detail("This will remove all cached image hashes. They will be recomputed on the next scan. This cannot be undone.")
    ...
    import_btn.connect_clicked(clone!(#[strong] window, #[strong] results_box, #[strong] results_data,
        ...
        #[strong] ref_dirs, #[strong] per_file_refs, move |_| {
        let dialog = gtk4::FileDialog::new();
    ...
    about_btn.connect_clicked(clone!(#[strong] window, move |_| {
...
struct FileData {
    ...
    #[allow(dead_code)]
    is_deleted: std::cell::RefCell<bool>,
}
...
fn is_ref_path(path: &str, ref_dirs: &Mutex<HashSet<String>>) -> bool {
    ...
    rd.iter().any(|d| path.starts_with(d))
}
...
fn build_results(results_box: &gtk4::Box,
    ...
    for (_gi, group) in groups.iter().enumerate() {
        ...
        for (_fi, entry) in group.files.iter().enumerate() {
            ...
            let deleted_label = gtk4::Label::new(Some("→ File Moved To Trash"));
            ...
            let ref_badge = gtk4::Label::new(Some("Reference Image"));
            ...
            move_btn.connect_clicked(clone!(#[strong] path_str, #[weak] status_label, move |_| {
                if !check_clone.is_active() {
                    status_label.set_text("Select the file to move");
                    return;
                }
                move_single_file(&path_str, &status_label, &pw);
            }));
            trash_btn.connect_clicked(clone!(#[strong] path_str, #[weak] status_label,
            ...
            file_datas.push(FileData {
                ...
                is_deleted: std::cell::RefCell::new(false),
            });
        }
...
fn apply_select_by(data: &[GroupData],
    ...
    let (dim_tx, dim_rx) = std::sync::mpsc::channel::<HashSet<String>>();
...
fn move_single_file(path: &str, status_label: &gtk4::Label, window: &impl IsA<gtk4::Window>) {
    let dialog = gtk4::FileDialog::new();
    dialog.set_title("Select destination folder");
    let p = path.to_owned();
    let sl = status_label.clone();
...
fn show_group_preview(
    ...
    let main_checkboxes: HashMap<String, gtk4::CheckButton> = results_data
        .lock()
        .unwrap()
        .iter()
        .flat_map(|gd| gd.files.iter())
        .map(|fd| (fd.path.clone(), fd.check.clone()))
        .collect();
    ...
    for (path, is_ref) in entries {
        ...
        check.set_active(main_checkboxes.get(path).map(|cb| cb.is_active()).unwrap_or(false));
        let checkbox_map = main_checkboxes.clone();
        let p = path.clone();
        check.connect_toggled(move |cb| {
            if let Some(main_cb) = checkbox_map.get(&p) {
                main_cb.set_active(cb.is_active());
            }
        });
        ...
        let sl = status_label.clone();
        let pw = window.clone();
        let move_check = check.clone();
        move_btn.connect_clicked(move |_| {
            if !move_check.is_active() {
                sl.set_text("Select the file to move");
                return;
            }
            move_single_file(&path_for_move, &sl, &pw);
        });
        ...
        let trash_check = check.clone();
        trash_btn.connect_clicked(move |_| {
            if !trash_check.is_active() {
                sl2.set_text("Select the file to trash");
                return;
            }
...
fn show_about_window(parent: &gtk4::Window) {
    let about = libadwaita::AboutDialog::builder()
        ...
        .version(APP_VERSION)
        .comments("Duplicate Image Finder")
        .developer_name("ImpHash")
        .issue_url("https://github.com/anomalyco/opencode/issues")
        .copyright("\u{00a9} 2024 ImpHash")
        .build();
