...
fn dhash_from_gray(gray: &image::GrayImage) -> u64 {
...
pub fn dhash(path: &Path, cancel: &AtomicBool) -> Result<u64> {
...
pub fn dhash_rotations(path: &Path, cancel: &AtomicBool) -> Result<[u64; 4]> {
    let img = open_image_timeout(path, cancel)?;
    let gray = img.to_luma8();

    let h0 = dhash_from_gray(&gray);
    ...
    let h90 = dhash_from_gray(&image::imageops::rotate90(&medium));
    let h180 = dhash_from_gray(&image::imageops::rotate180(&medium));
    let h270 = dhash_from_gray(&image::imageops::rotate270(&medium));

    Ok([h0, h90, h180, h270])
}

/// Minimum Hamming distance across all 16 pairs of 4 rotation hashes.
...
mod tests {
    ...
    fn test_dhash_duplicate_images() {
        let h1 = dhash(Path::new("/tmp/imphash_test/photo1.png"), &CANCEL_FALSE).unwrap();
        let h2 = dhash(Path::new("/tmp/imphash_test/photo1_copy.png"), &CANCEL_FALSE).unwrap();
    ...
    fn test_dhash_different_images() {
        let h1 = dhash(Path::new("/tmp/imphash_test/photo1.png"), &CANCEL_FALSE).unwrap();
        let h2 = dhash(Path::new("/tmp/imphash_test/photo2.png"), &CANCEL_FALSE).unwrap();
        assert_ne!(h1, h2, "different images should have different hashes");
    }

    #[test]
    fn test_dhash_compression_levels() {
        let h_high = dhash(Path::new("/tmp/imphash_test/test_highq.jpg"), &CANCEL_FALSE).unwrap();
        let h_low = dhash(Path::new("/tmp/imphash_test/test_lowq.jpg"), &CANCEL_FALSE).unwrap();
        let h_png = dhash(Path::new("/tmp/imphash_test/test_original.png"), &CANCEL_FALSE).unwrap();
    ...
    fn test_dhash_natural_images_compression() {
        let h_high = dhash(Path::new("/tmp/imphash_test2/natural_highq.jpg"), &CANCEL_FALSE).unwrap();
        let h_low = dhash(Path::new("/tmp/imphash_test2/natural_lowq.jpg"), &CANCEL_FALSE).unwrap();
        let h_med = dhash(Path::new("/tmp/imphash_test2/natural_medium.jpg"), &CANCEL_FALSE).unwrap();
        let h_png = dhash(Path::new("/tmp/imphash_test2/natural_png.png"), &CANCEL_FALSE).unwrap();
    ...
    fn test_dhash_alpha_channel() {
        let h_rgba = dhash(Path::new("/tmp/imphash_test3/rgba.png"), &CANCEL_FALSE).unwrap();
        let h_rgb_jpg = dhash(Path::new("/tmp/imphash_test3/rgb.jpg"), &CANCEL_FALSE).unwrap();
        let h_rgb_png = dhash(Path::new("/tmp/imphash_test3/rgb.png"), &CANCEL_FALSE).unwrap();
    ...
    fn test_dhash_different_dimensions() {
        let h_full = dhash(Path::new("/tmp/imphash_test3/halfsize.jpg"), &CANCEL_FALSE).unwrap();
        let h_half = dhash(Path::new("/tmp/imphash_test3/halfsize_small.jpg"), &CANCEL_FALSE).unwrap();
    ...
    fn test_dhash_large_high_quality() {
        let h_tiff_uncomp = dhash(Path::new("/tmp/imphash_test4/large_uncompressed.tiff"), &CANCEL_FALSE).unwrap();
        let h_tiff_comp = dhash(Path::new("/tmp/imphash_test4/large_compressed.tiff"), &CANCEL_FALSE).unwrap();
        let h_jpg_high = dhash(Path::new("/tmp/imphash_test4/large_highq.jpg"), &CANCEL_FALSE).unwrap();
        let h_jpg_low = dhash(Path::new("/tmp/imphash_test4/large_lowq.jpg"), &CANCEL_FALSE).unwrap();
        let h_png = dhash(Path::new("/tmp/imphash_test4/large_png.png"), &CANCEL_FALSE).unwrap();
    ...
    fn test_dhash_color_gamut() {
        let h_srgb = dhash(Path::new("/tmp/imphash_test3/srgb.jpg"), &CANCEL_FALSE).unwrap();
        let h_adobergb = dhash(Path::new("/tmp/imphash_test3/adobergb.jpg"), &CANCEL_FALSE).unwrap();
